...
impl 命令 for 记忆命令 {
    ...
    fn 执行(&self, 参数: &[&str]) -> 命令结果 {
        use crate::记忆读取_殿::{
            世界快照, 事件流_全部, 事件流_记录, 事件流_读取, 地道精炼, 完成度自评, 播种格位36,
            查会话, 查全部记忆, 格位统计, 终点归档, 读取_三档投影, 默认记忆库路径,
        };
        ...
        match 子 {
            ...
            "世界快照" => {
                ...
                match 世界快照(库, 测试数, 套件数) {
                    ...
                    Err(e) => 命令结果::失败(1, format!("世界快照失败：{}", e)),
                }
            }
            "统计" => {
                let 库 = if 参数.len() >= 2 && !参数[1].is_empty() {
            ...
            _ => 命令结果::失败(
                1,
                "用法：记忆 <三档|查|播种|精炼|会话|事件|完成度|统计|世界快照> [参数] [库路径]",
            ),
        }
    }
}
