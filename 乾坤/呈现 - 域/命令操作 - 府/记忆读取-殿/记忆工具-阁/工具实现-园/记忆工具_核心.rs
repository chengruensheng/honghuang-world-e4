...
/// 世界快照：把客观事实（测试数/套件数）登记为 世界/验收 行档（decided_by=扫描）
///
/// 兑现「测试数/套件数/版本号等应落库登记而非人工维护」（防漂移）：
/// 每次全量验收后运行一次，世界/验收 链头即最新数值；重复运行不产生重复链头。
pub fn 世界快照(
    记忆库路径: &str,
    测试数: &str,
    套件数: &str,
...
///
/// ①完成度自评（刻度+依据 → 经历/验收）②终点归档（查会话轨迹 → 地道精炼
/// 教训/机制/波及 → 经历）。返回收尾日志。
pub fn 任务收尾(
    记忆库路径: &str,
    任务标识: &str,
    ...
    // 收尾仪式补事件流：任务完成时序事实（审计轨迹完整，append-only 不删除）
    let 事件类型 = "任务收尾";
...
mod 测试 {
    ...
    fn 完成度自评_刻度合法与拒绝() {
        ...
        let _ = std::fs::remove_file(&库);
        let _id = 完成度自评(&库, "播种", 4, "23 合法格位补齐").unwrap();
    ...
    fn 世界快照_链头更新且不重复() {
        ...
        assert!(
            验收.iter().any(|s| s.contains("套件数=32")),
            "世界/验收 应含套件数=32"
        );
        // 重复运行：链头更新为 336，且 336 只出现 1 次
        世界快照(&库, "336", "32").unwrap();
        let 验收2 = 读格位仓库(&库, 范畴::世界, 阶段::验收);
        assert_eq!(
        ...
        assert!(
            验收2.iter().any(|s| s.contains("测试数=336")),
            "链头应更新为 336"
        );
        let _ = std::fs::remove_file(&库);
    }
    ...
    fn 任务收尾_完成度与精炼一步到位() {
        ...
        assert_eq!(收.len(), 3, "收尾日志应 3 条：{}", 收.join(";"));
        assert!(收[2].contains("事件流"), "第三条应为事件流：{}", 收[2]);
        let 事件 = 事件流_全部(&库).unwrap();
        assert!(
            事件.iter().any(|s| s.contains("任务收尾")
                && s.contains("完成度 4/5")
                && s.contains("流水线X")),
            "事件流应含收尾事件：{}",
            事件.join(";")
        ...
        let 全部 = 查全部记忆(&库);
        assert!(
            全部.iter().any(|s| s.contains("测试数增加")),
            "精炼行应含执行结果：{}",
            全部.join(";")
        );
        let _ = std::fs::remove_file(&库);
