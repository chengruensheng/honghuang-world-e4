...
fn 分发生产(参数: &[&str]) -> 命令结果 {
    ...
    match 参数[0] {
        ...
        "真任务" | "real-task" => {
            ...
            // --落盘 走确定性自举流水线（真实落盘 + cargo 验证，根治「准圣 0/14 打回」）
...
///
/// 执行器双模式：
/// - 默认（玩具流水线）：四角色接力产出验收文本，**不落盘**（展示完整流式过程用）。
/// - `--落盘`（确定性自举流水线）：大罗产出代码 → 确定性执行器真实覆写「固定挂载点」
///   生成区.rs（道韵/律法-域/规则-府/生成-验证-殿/示例-生成-阁/生成-落地-园/）→
///   include! 静态挂载到 生成验证入口 example → `cargo test -p guize_fu --example 生成验证入口`
///   必然编译并真实执行生成区内的 #[test] 测试 → 准圣拿到真实验收证据验收 → 道祖终裁
///   （验证不过硬门槛改判打回，根治「0/14 打回」：LLM 代码有真实编译+执行证据才交付）。
///
/// 安全边界：`--落盘` 只覆写固定挂载点生成区.rs（治理白名单放行路径），
/// 治理资产（乾坤/鸿蒙/传承殿 等）由 gongju_fu 写路径黑名单拦截。
fn 真任务(参数: &[&str]) -> 命令结果 {
    use mingling_caozuo_fu::{
        写入任务材料, 材料模板_开发任务, 默认记忆库路径
    };
    ...
    let 落盘模式 = 参数.contains(&"--落盘") || 参数.contains(&"--disk");
    ...
    // 确定性自举模式（--落盘）：真实覆写固定挂载点 + cargo test 验证，根治「准圣 0/14 打回」
    if 落盘模式 {
        // 目标文件 = 固定挂载点生成区.rs（include! 静态挂载，cargo 必然编译执行）
        let 目标文件 = 取(
            "--目标文件=",
            "道韵/律法-域/规则-府/生成-验证-殿/示例-生成-阁/生成-落地-园/生成区.rs",
        );
        // 验收命令：cargo test 指定 example，真实执行生成区内的 #[test]
        let 验收命令 = 取(
            "--验收命令=",
            "cargo test -p guize_fu --example 生成验证入口",
        );
        let 可证伪命题 = format!(
            "cargo test 退出码 0 且 {} 含函数「{}」与测试「{}」",
            目标文件, 函数名, 测试名
        );
        // 工单追加落盘约束：产物须能被固定挂载点 include 编译（只含测试与辅助 fn、全中文标识符）
        let 落盘工单 = format!(
            "{}\n\n[落盘约束] 产出代码将覆写到固定挂载点（生成区.rs）经 include! 编译验证，\n必须：① 含 #[test] 测试「{}」断言 {}（含边界用例）；② 函数/测试名全中文标识符；\n③ 只写函数与测试定义（不得写 fn main、不得写 use 语句、不得写模块级文档注释 //!）。",
            工单, 测试名, 断言
        );
        let mut 单参数 = std::collections::HashMap::new();
        单参数.insert("标识".to_string(), 任务标识.clone());
        单参数.insert("目标文件".to_string(), 目标文件.clone());
        单参数.insert("需求描述".to_string(), 落盘工单.clone());
        单参数.insert("验收命令".to_string(), 验收命令.clone());
        单参数.insert("可证伪命题".to_string(), 可证伪命题);
        单参数.insert("decided_by".to_string(), "界主".to_string());
        let 单 = match 自举任务单::从参数解析(&单参数) {
            Ok(单) => 单,
            Err(e) => return 命令结果::失败(2, format!("真任务：任务单解析失败：{}\n", e)),
        };
        let mut 结果 = mingling_caozuo_fu::跑流水线_自举(&单);
        // 任务后写程序/实施记忆 + 刷新永驻摘要（真实生产留痕可审计）
        if 结果.退出码 == 0 {
            if let Err(错) = 写入任务记忆(
                默认记忆库路径,
                &format!("真任务(落盘)完成：{}", 任务标识),
                &工单,
            ) {
                结果.输出.push_str(&format!("[记忆写入失败] {}\n", 错));
            }
    ...
    // 默认玩具流水线：展示完整流式过程（四角色接力产出验收文本，不落盘）
    let mut 结果 = mingling_caozuo_fu::跑流水线_mock_llm(&工单);
    // 任务后写程序/实施记忆 + 刷新永驻摘要（真实生产留痕可审计）
    if 结果.退出码 == 0 {
        if let Err(错) =
...
mod 真任务测试 {
    ...
    #[test]
    fn 测试_真任务_落盘分支_任务单构造() {
        // 验证 --落盘 分支能构造出自举任务单（不触发真实 LLM 网络调用）。
        // 无法直接测 真任务()（内部会跑真实/模拟流水线），此处等价验证：
        // 1. --落盘 被识别；2. 目标文件/验收命令 默认值正确可解析。
        let 参数: Vec<&str> = vec!["真任务", "--落盘"];
        let 取 = |前缀: &str, 默认: &str| -> String {
            参数
                .iter()
                .find_map(|a| a.strip_prefix(前缀))
        ...
        assert!(
            目标文件.contains("生成-落地-园/生成区.rs"),
            "默认目标文件应落固定挂载点生成区.rs：{}",
            目标文件
        );
        assert!(
            验收命令.contains("cargo test"),
            "验收命令应为 cargo test：{}",
            验收命令
        );

        // 任务单解析（对齐 真任务 --落盘 分支内部逻辑）
        use renwu_zhixing_fu::自举任务单;
        let mut 单参数 = std::collections::HashMap::new();
        单参数.insert("标识".to_string(), "真实开发任务-整数加法".to_string());
        单参数.insert("目标文件".to_string(), 目标文件.clone());
        单参数.insert("需求描述".to_string(), "工单".to_string());
        单参数.insert("验收命令".to_string(), 验收命令.clone());
        单参数.insert("可证伪命题".to_string(), "cargo check 退出码 0".to_string());
        单参数.insert("decided_by".to_string(), "界主".to_string());
        let 单 = 自举任务单::从参数解析(&单参数).expect("落盘分支任务单应可解析");
        assert_eq!(单.目标文件, 目标文件);
        assert_eq!(单.验收命令, 验收命令);
    }
}
