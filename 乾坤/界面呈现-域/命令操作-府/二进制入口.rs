...
/// - `--落盘`（确定性自举流水线）：大罗产出代码 → 确定性执行器真实落盘到 `--目标文件=`
///   （默认 道韵/自举验证-园/自举生成_临时.rs，验收 `cargo check`）→ cargo 验证 →
///   准圣拿到真实验收证据验收 → 道祖终裁。验证不过硬门槛改判打回（根治「0/14 打回」：
///   LLM 产代码文本但无落盘证据，准圣无法验收）。
///
/// 安全边界：`--落盘` 只允许写「生成目录」（道韵/自举验证-园/ 等），
/// 绝不落盘到治理资产（乾坤/鸿蒙/传承殿 等）——确定性执行器的路径白名单在 gongju_fu 侧拦截。
fn 真任务(参数: &[&str]) -> 命令结果 {
    use mingling_caozuo_fu::{
    ...
    // 确定性自举模式（--落盘）：真实落盘 + cargo 验证，根治「准圣 0/14 打回」
    if 落盘模式 {
        // 目标文件默认落在 自举验证-园（生成目录，非治理资产；治理资产由路径白名单拒绝）
        let 目标文件 = 取("--目标文件=", "道韵/自举验证-园/自举生成_临时.rs");
        // 验收命令归一化为 cargo check（自举执行内把 build→check：自指锁免疫 + 类型检查等价）
        let 验收命令 = 取("--验收命令=", "cargo check");
        let 可证伪命题 = format!(
            "cargo check 退出码 0 且 {} 含函数「{}」与测试「{}」",
            目标文件, 函数名, 测试名
        );
        let mut 单参数 = std::collections::HashMap::new();
        单参数.insert("标识".to_string(), 任务标识.clone());
        单参数.insert("目标文件".to_string(), 目标文件.clone());
        单参数.insert("需求描述".to_string(), 工单.clone());
...
mod 真任务测试 {
    ...
    fn 测试_真任务_落盘分支_任务单构造() {
        ...
        let 验收命令 = 取("--验收命令=", "cargo check");
        assert!(
            目标文件.contains("道韵/自举验证-园"),
            "默认目标文件应落自举验证-园：{}",
            目标文件
        );
        assert!(!验收命令.is_empty(), "验收命令不应为空");
