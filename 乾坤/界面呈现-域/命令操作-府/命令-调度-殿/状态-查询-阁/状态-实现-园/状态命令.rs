//! 状态-实现-园 - Status 命令实现（实时快照，非硬编码）
//!
//! 决策锚：260828-自动状态报告（第五十三轮）
//! falsifiable：status 输出含「最近提交」「未提交/未跟踪」「工作空间」三节，
//!             且工作空间府数 = 根 Cargo.toml members 路径行数（当前 23），非硬编码常量。
...
mod 测试 {
    ...
    /// 防退化：工作空间府数必须与根 Cargo.toml members 路径行数一致（当前 22），
    /// 杜绝状态报告退回硬编码假数字。
    #[test]
    fn 府数_读根Cargo_toml为22() {
        let 根 = Path::new("E:\\洪荒 - 世界");
        assert_eq!(
            府数(根),
            22,
            "工作空间府数应为 22（根 Cargo.toml members 路径行数）"
        );
    }
