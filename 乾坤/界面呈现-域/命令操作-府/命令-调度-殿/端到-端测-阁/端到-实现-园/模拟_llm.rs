//! 端到-实现-园 - 后端选择端到端（v4 阶段 17 + Round 9）
//!
...
/// LLM_STREAM 开关：默认开，off 关闭逐字流式显示（落盘不受影响）
fn 流式开关开() -> bool {
...
impl moxing_fu::模型连接 for 连接抽象 {
    ...
    fn 发送_流式(
        ...
        match self {
            // Mock 走默认实现（回退非流式，一次性回调完整内容，行为不变）
            连接抽象::Mock(c) => c.发送_流式(配置, 请求, 回调),
            // HTTP 走真实 SSE 流式（逐 delta 回调）
            连接抽象::HTTP(c) => c.发送_流式(配置, 请求, 回调),
            #[cfg(test)]
            连接抽象::故障(e) => Err(e.clone()),
        }
    }
...
fn 跑流水线_按后端(任务标识: &str, 模式: 后端模式) -> 命令结果 {
    ...
    // 交付回填库：
    // - 真实/默认模式：交付后回填到默认持久库（真实运行留痕，人类可审计）
    // - Mock 模式：不回填（mock 产物是假的，绝不登记/交付，绝不碰默认库）
    //   [治理] mock 若回退到默认库，事件流会被测试痕迹污染，人类无法分辨真实与测试残留
...
fn 跑流水线_自举核心(
    ...
    loop {
        ...
        for 池名 in 池顺序.iter() {
            ...
            match 调用_流式_带重试(调用器, 池名, &req, &mut 回调) {
                Ok(响应) => {
                    上文 = 响应.内容.clone();
                    流式尾(池名);
...
/// 流式调用容错（与 调用_带重试 同契约）：网络抖动类错误幂等重试一次，边收边回调增量正文
/// 幂等性：同请求重发不改变语义；重试时回调会再次触发（可接受：重试罕见，重复输出优于丢内容）
pub(crate) fn 调用_流式_带重试<C: moxing_fu::模型连接>(
    调用器: &moxing_fu::LLM调用器<C>,
    池名: &str,
    请求: &moxing_fu::请求,
    回调: &mut dyn FnMut(&str),
) -> Result<moxing_fu::响应, moxing_fu::错误> {
    match 调用器.调用_流式(池名, 请求, 回调) {
        Ok(响应) => Ok(响应),
        Err(首次错误) => {
            if 是否可重试(&首次错误) {
                调用器.调用_流式(池名, 请求, 回调)
...
fn 跑流水线_按连接(
    ...
    实时进度(&format!("任务「{}」启动，召回记忆上下文…", 任务标识));
    ...
    // [治理] mock 模式回填库为 None，此时必须用临时隔离库而非默认库——
    //   否则 mock 测试的事件流（打回/终裁）会写入默认真实库，污染审计流，人类无法分辨真实与测试残留。
    let 读库 = match 回填库 {
        Some(库) => 库.to_string(),
        None => {
        ...
        for 池名 in 池顺序.iter() {
            ...
            match 调用_流式_带重试(调用器, 池名, &req, &mut 回调) {
                Ok(响应) => {
                    ...
                    // 完整内容落盘（流式显示与落盘解耦：显示逐字实时，落盘保证可追溯）
                    流式落盘(池名, &响应.内容);
                }
                Err(e) => {
                    流式尾(池名);
    ...
    实时进度(&format!("流水线完成，LLM 失败数={}", llm失败数));
    if llm失败数 > 0 {
...
mod 测试 {
    ...
    #[test]
    fn 测试_mock_不污染默认真实库() {
        // [治理回归] mock 模式（回填库=None）不得向默认真实库写任何事件流/记忆：
        //   否则人类查看审计流时无法分辨真实运行与测试残留（决策锚：默认=真实，严禁降级污染）
        let _g = 环境锁();
        清空_env();
        std::env::set_var("LLM_BACKEND", "mock");

        // 用临时库模拟「默认真实库」，跑前记录事件流行数
        let 模拟默认库 = std::env::temp_dir()
            .join("洪荒mock_回归_默认库.sq3")
            .to_str()
            .unwrap()
            .to_string();
        let _ = std::fs::remove_file(&模拟默认库);
        // 先建库（写入一条基线事件，模拟已有人类真实运行痕迹）
        crate::事件流_记录(&模拟默认库, "真实运行痕迹", "人类可审计基线")
            .expect("基线事件写入失败");
        let 跑前事件流 = crate::事件流_全部(&模拟默认库).unwrap().len();

        // mock 流水线（回填库 None → 读库应走临时隔离库，绝不碰默认库）
        let 调用器 =
            moxing_fu::LLM调用器::新建(构建模拟池(), 连接抽象::Mock(MockLLM连接::新建()));
        let r = 跑流水线_按连接("mock污染回归", &调用器, String::new(), None, 3);
        assert_eq!(r.退出码, 0, "mock 流水线应正常完成：{}", r.输出);

        // 默认库事件流行数不得增长（mock 不得写入任何痕迹）
        let 跑后事件流 = crate::事件流_全部(&模拟默认库).unwrap().len();
        assert_eq!(
            跑后事件流, 跑前事件流,
            "mock 模式污染了默认真实库事件流（{} → {}）：{}",
            跑前事件流, 跑后事件流, r.输出
