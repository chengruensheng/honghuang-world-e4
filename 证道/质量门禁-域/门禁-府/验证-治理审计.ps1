...
$存储核心 = "鸿蒙/基础设施-域/记忆承载-府/记忆-存储-殿/SQLite-存储-阁/SQLite-实现-园/SQLite_核心.rs"
if (Test-Path $存储核心) {
...
$模拟 = "乾坤/界面呈现-域/命令操作-府/命令-调度-殿/端到-端测-阁/端到-实现-园/模拟_llm.rs"
if (Test-Path $模拟) {
...
$闭环 = "鸿蒙/基础设施-域/记忆承载-府/记忆-应用-殿/闭环-应用-阁/闭环-实现-园/闭环_核心.rs"
if (Test-Path $闭环) {
...
$分类核心 = "鸿蒙/基础设施-域/流水线驱动-府/流水-分类-殿/分类-规则-阁/分类-实现-园/分类_核心.rs"
if (Test-Path $分类核心) {
