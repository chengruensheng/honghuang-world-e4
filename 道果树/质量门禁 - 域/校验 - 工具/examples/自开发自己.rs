...
fn 任务_prompt(失败列表: &[(u8, String, String)], 当前源: &str) -> String {
    ...
    s.push_str("2. 路径 `\"道果树/质量门禁-域/门禁-府\"` 没有空格 —— 实际目录是 `\"道果树/质量门禁 - 域/门禁 - 府\"`，补全空格。\n");
    ...
    s.push_str("=== FILE: 道果树/质量门禁 - 域/校验 - 工具/架构校验-殿/结构校验-阁/架构实现-园/架构校验.rs ===\n");
    s.push_str("```rust\n<完整文件内容>\n```\n\n");
    s.push_str("禁止：\n");
