...
use crate::事件持久化_殿::{反序列化行, 序列化为行};
use crate::事件类型_殿::{事件, 算事件哈希, 错误, 零哈希, Hash};

// 阁符号引用：瀑布阁/串行阁在分发殿下（六层返工后改用 crate:: 路径）
use crate::事件分发_殿::串行分发_阁::{Serial监听器, Serial结果};
use crate::事件分发_殿::瀑布分发_阁::Waterfall监听器;
...
mod 测试 {
    ...
    use crate::事件类型_殿::{事件类型, 分发模式, 算哈希};
